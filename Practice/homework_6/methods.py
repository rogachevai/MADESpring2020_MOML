...
class BFGS(object):
    ...
    def calc_H(self):
        ...
        ro = self.s_k.flatten().dot(self.y_k.flatten())
        c = 1e-4
        self.H_k -= self.H_k.dot(self.y_k).dot(self.s_k.T) / (ro + c)
        self.H_k -= (self.H_k.dot(self.y_k).dot(self.s_k.T) / (ro + c)).T
        self.H_k += self.s_k.dot(self.s_k.T) / (ro + c)
...
class LBFGS(object):
    ...
    def lbfgs_mul(self, v, memory, gamma):
        if len(memory) == 0:
            return gamma * v
        s, y = memory[-1]
        v_next = v - s.dot(v) / y.dot(s) * y
        z = self.lbfgs_mul(v_next, memory[:-1], gamma)
